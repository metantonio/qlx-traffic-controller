...
from .models import ManagedWorkbook, UserState
import openpyxl
import pandas as pd
...
from .sessions import sessions, with_workbook, with_workbook_mutation, with_session
from .schemas import (
...
def open_workbook(
    state: UserState,
    operation: Annotated[
        OpenWorkbookOperation, Field(description="Open workbook operation parameters")
    ],
) -> str:
    """
    Open an Excel or CSV file to start a session.
    
    This must be called before using any other Excel tools.
    ...
    if not path.exists():
        return ToolError(f"File not found: {filepath}", code="FILE_NOT_FOUND").to_json()
    
    try:
        if path.suffix.lower() == ".csv":
            # Load CSV using pandas and convert to openpyxl
            ...
            state.workbook = ManagedWorkbook(
                wb=wb,
                filepath=filepath,
                cached_values_wb=None # No cached values for CSV conversion
            )
            return f"Successfully loaded CSV '{filepath}' into a new session."
        else:
            # Load Excel file
            state.workbook = ManagedWorkbook(
                wb=openpyxl.load_workbook(filepath),
            ...
            return f"Successfully loaded workbook '{filepath}' into the session."
    except Exception as e:
        return ToolError(f"Error loading file: {e}").to_json()
...
def delete_conditional_formatting_rule(
    ...
    operation: Annotated[
        DeleteConditionalFormattingOperation,
        Field(description="Delete conditional formatting operation parameters"),
    ],
) -> str:
    """
...
def delete_data_validation_rule(
    ...
    operation: Annotated[
        DeleteDataValidationOperation,
        Field(description="Delete data validation operation parameters"),
    ],
) -> str:
    """
