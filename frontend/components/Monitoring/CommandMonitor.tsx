...
export default function CommandMonitor({ events }: CommandMonitorProps) {
    const commandEvents = events.filter((e) => e.type === "tool_requested" || e.type === "security_alert");
                    ...
                    {commandEvents.map((event, idx) => (
                        <div key={idx} className="flex flex-col gap-1 border-b border-neutral-800/60 pb-3 last:border-0 hover:bg-neutral-900/30 rounded px-2 py-1 transition-colors">
                            <div className="flex items-center justify-between text-xs">
                                ...
                                <span className={`px-2 py-0.5 rounded ${event.type === 'security_alert' ? 'bg-red-500/20 text-red-400' : 'bg-emerald-500/10 text-emerald-400'}`}>
                                    {event.source}
                                </span>
                            </div>
                            <div className={`${event.type === 'security_alert' ? 'text-red-300' : 'text-neutral-300'} break-all`}>
