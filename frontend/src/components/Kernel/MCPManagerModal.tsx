...
export default function MCPManagerModal({ isOpen, onClose, onChanged }: MCPManagerModalProps) {
    const [activeTab, setActiveTab] = useState<'active' | 'store'>('active');
    ...
    const fetchStore = React.useCallback(async () => {
        try {
            const res = await fetch(`${apiUrl}/api/store/mcp`);
            const data = await res.json();
            setStoreItems(data);
        } catch (err) {
            console.error("Failed to fetch MCP store:", err);
        }
    }, [apiUrl]);
    ...
    }, [isOpen, fetchServers, fetchStore]);
    ...
    const handleInstall = async (id: string, requiresKey: boolean) => {
        let overrides: Record<string, string> = {};
        if (requiresKey) {
            const key = prompt(`Please enter the API Key for ${id}:`);
            if (!key) return;
        ...
        setInstallingId(id);
        try {
            const res = await fetch(`${apiUrl}/api/store/install`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    server_id: id,
            ...
            <div className="bg-neutral-900 border border-neutral-800 w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex border-b border-neutral-800 px-6">
                    <button
                        ...
                        className={`py-3 px-4 text-xs font-bold uppercase tracking-widest transition-all border-b-2 ${activeTab === 'active' ? 'border-blue-500 text-blue-400' : 'border-transparent text-neutral-500 hover:text-neutral-300'}`}
                    >
                        Active Bridges
                    </button>
                    <button
                        onClick={() => setActiveTab('store')}
                        className={`py-3 px-4 text-xs font-bold uppercase tracking-widest transition-all border-b-2 ${activeTab === 'store' ? 'border-blue-500 text-blue-400' : 'border-transparent text-neutral-500 hover:text-neutral-300'}`}
                    >
                        Store
                    </button>
                </div>
                ...
                <div className="p-6 space-y-8 max-h-[60vh] overflow-y-auto custom-scrollbar">
                    {activeTab === 'active' ? (
                        ...
                        <div className="space-y-4">
                            <h3 className="text-xs font-bold text-neutral-500 uppercase tracking-widest">Community Store</h3>
                            <div className="grid grid-cols-1 gap-3">
                                {Object.entries(storeItems).map(([id, item]) => {
                                    const isInstalled = servers.some(s => s.id === id);
                                    return (
                                        <div key={id} className="p-4 bg-neutral-800/30 border border-neutral-800/50 rounded-2xl flex items-center justify-between hover:border-neutral-700 transition-all">
                                            <div className="flex items-center gap-4">
                                            ...
                                            {isInstalled ? (
                                                ...
                                                <button
                                                    onClick={() => handleInstall(id, item.requires_api_key)}
                                                    disabled={installingId === id}
                                                    className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold uppercase tracking-widest rounded-xl transition-all disabled:opacity-50"
                                                >
                                ...
                                <div className="p-5 bg-neutral-950/50 border border-neutral-800/50 rounded-2xl animate-in fade-in slide-in-from-top-2 duration-300">
                                    <div className="flex items-start gap-3 mb-4">
                                        ...
                                        <p className="text-[11px] text-neutral-400 leading-relaxed font-medium">
                                            ...
                                            You can find pre-configured ones in the <b>Store</b> tab.
                                        </p>
                                    </div>
                                </div>
                            )}
                ...
                <div className="p-4 border-t border-neutral-800 bg-neutral-950/50 flex justify-center">
                    ...
                    >
                        Browse External Registry <ExternalLink size={10} />
                    </a>
                </div>
            </div>
        </div>
